...
testudo_img = cv2.imread('AR-tag/docs/testudo.png')
...
Frame=0/home/raul/Documents/ENPM673/Opencv/Project-1/][1]))
            x,y = i.ravel()
        ...
        testudo_img=rotate_image(testudo_img,0)
        testudo_coords=get_world_coords(testudo_img)
        matrix = get_homography(corner_points,world_coords_ref_tag)
        warped_frame=get_warp_perspective(testudo_img,np.linalg.inv(matrix), (img.shape[1],img.shape[0]))
        kernel=np.ones((3,3))
